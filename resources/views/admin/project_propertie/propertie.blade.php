...
@if(session('success'))
    ...
    @endif
    <section>
        <p class="heading_for_admin_section">Add Project Properties 
            <a class="add_new_button m-0" href="{{ route('project_properties.create') }}">+ Add New Property</a>
            <a class="add_new_button  m-0" href="{{ route('property-type.create','project') }}">+ Add Property Type</a></p>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @if(isset($item->slug) && !empty($item->slug) && $item->slug)
                                <a class="edit_button" target="_blank" href="{{ route('devlopment.detail_page', $item->slug) }}">
                                    <i class="fa-solid fa-eye"></i> View
                                </a>
                            @endif
                            <a class="edit_button" href="{{ route('project_properties.edit', $item) }}">
                                <i class="fa-solid fa-pencil"></i>
                            </a>
                            <form action="{{ route('project_properties.destroy', $item) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
