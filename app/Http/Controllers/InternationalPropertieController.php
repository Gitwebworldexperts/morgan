...
class InternationalPropertieController extends Controller
{
    /**
    ...
    {
        $this->property = 'international';
        // $this->middleware('auth');
    } 
        public function index()
        {
            $propertie = InternationalPropertie::orderBy('id', 'desc')->with('propertyType')->get();
            return view('admin.international_propertie.propertie', compact('propertie'));
         }
        ...
        public function create()
        {
            ...
            if(!count($region)){
                return redirect()->back()->with('error', 'Please create region');
            }
        $agents = Agent::all();
        $amenitie = Amenitie::where('status','1')->get();
            return view('admin.international_propertie.create',compact('propertyTypes','amenitie','agents','region'));
        }
        ...
        $slug = generateSlug($request->name."_international", \App\Models\InternationalPropertie::class);
            // Create a new property instance
            $property = new InternationalPropertie();
        ...
        $property->information_button_url_2 = $request->eighth_section_button_3_2;
            ...
            if ($request->hasFile('images')) {
                foreach ($request->file('images') as $key => $image) {
                    ...
                    $banner->page_type = 'international'; // Set the appropriate page_id if needed
            ...
            if ($request->hasFile('featured_image')) {
            	...
            	$banner->page_type = 'international';
                $banner->property_id = $property->id; // Associate with the newly created property
                $banner->save();
            }
        ...
        {
            $property = InternationalPropertie::findOrFail($id);
            if(!$property){
        ...
        $amenitie = Amenitie::where('status','1')->get();
            $region = Region::all();
            return view('admin.international_propertie.edit', compact('property','propertyTypes','agents','amenitie','region'));
        }
