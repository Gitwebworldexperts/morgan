...
class RentPropertieController extends Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        {
            $propertie = RentPropertie::orderBy('id', 'desc')->with('propertyType')->get();
            return view('admin.rent_propertie.propertie', compact('propertie'));
        }
        ...
        $amenitie = Amenitie::where('status','1')->get();
            return view('admin.rent_propertie.create',compact('propertyTypes','amenitie','agents'));
        }
        ...
        $slug = generateSlug($request->name."_rent", \App\Models\RentPropertie::class);
            // Create a new property instance
            $property = new RentPropertie();
        ...
        $property->information_button_url_2 = $request->eighth_section_button_3_2;
            ...
            if ($request->hasFile('images')) {
                foreach ($request->file('images') as $key => $image) {
                    ...
                    $banner->page_type = 'rent'; // Set the appropriate page_id if needed
        ...
        {
            $property = RentPropertie::findOrFail($id);
            if(!$property){
        ...
        $agents = Agent::all();
        $amenitie = Amenitie::where('status','1')->get();
            return view('admin.rent_propertie.edit', compact('property','propertyTypes','agents','amenitie'));
        }
    ...
    {
         // temp
            // $buy = RentPropertie::all();
            // foreach($buy as $item){
            //     $property = RentPropertie::findOrFail($item->id);
            //     deletePropertyFiles($property,'rent');
            //     if(!$property){
            //         return redirect()->back()->with('error', 'Private property not found!');
            //     }
            //     $property->delete();
